...
class LineItem extends AbstractService {
	...
	public function getItemsFromSession(){
		...
		$this->total = $this->getSessionTotals();
		...
		if ($this->registry->get('currency')->getValue($currency_code) == 1) {
			...
			if (!\WalleeHelper::instance($this->registry)->areAmountsEqual($calculated_total, $expected_total, $currency_code)) {
				\WalleeHelper::instance($this->registry)->log(
						"Invalid order total calculated. Calculated total: $calculated_total, Expected total: $expected_total.", \WalleeHelper::LOG_ERROR);
	...
	private function getSessionTotals(){
		// Totals
		...
		$results = $this->registry->get('model_setting_extension')->getExtensions('total');
		foreach ($results as $key => $value) {
			$sort_order[$key] = $this->registry->get('config')->get('total_' . $value['code'] . '_sort_order');
		}
		
		array_multisort($sort_order, SORT_ASC, $results);
		
		foreach ($results as $result) {
			if ($this->registry->get('config')->get('total_' . $result['code'] . '_status')) {
				$this->registry->get('load')->model('extension/total/' . $result['code']);
