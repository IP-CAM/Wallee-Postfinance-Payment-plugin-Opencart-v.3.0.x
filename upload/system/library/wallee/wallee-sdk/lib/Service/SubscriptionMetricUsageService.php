...
/**
 * SubscriptionMetricUsageService service
 *
 * @category Class
 * @package  Wallee\Sdk
 * @author   wallee AG
...
class SubscriptionMetricUsageService {
	 ...
	 */
	public function create($space_id, $usage_report) {
		return $this->createWithHttpInfo($space_id, $usage_report)->getData();
	}
	 ...
	 * @return ApiResponse
	 */
	public function createWithHttpInfo($space_id, $usage_report) {
		// verify the required parameter 'space_id' is set
		if (is_null($space_id)) {
		...
		if (is_null($usage_report)) {
			throw new \InvalidArgumentException('Missing the required parameter $usage_report when calling create');
		}
		// header params
		$headerParams = [];
		$headerAccept = $this->apiClient->selectHeaderAccept(['application/json;charset=utf-8']);
		...
		// body params
		$tempBody = null;
		if (isset($usage_report)) {
			$tempBody = $usage_report;
		}
