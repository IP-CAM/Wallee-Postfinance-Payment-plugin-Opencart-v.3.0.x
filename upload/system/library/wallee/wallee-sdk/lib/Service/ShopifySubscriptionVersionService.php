...
/**
 * ShopifySubscriptionVersionService service
 *
 * @category Class
 * @package  Wallee\Sdk
 * @author   wallee AG
...
class ShopifySubscriptionVersionService {
	...
	public function countWithHttpInfo($space_id, $filter = null) {
		...
		// path params
		$resourcePath = '/shopify-subscription-version/count';
		...
		try {
			...
			$response = $this->apiClient->callApi(
				...
				$headerParams,
				'int',
				'/shopify-subscription-version/count'
			);
	...
	public function readWithHttpInfo($space_id, $id) {
		...
		// path params
		$resourcePath = '/shopify-subscription-version/read';
		// default format to json
		$resourcePath = str_replace('{format}', 'json', $resourcePath);
		...
		try {
			...
			$response = $this->apiClient->callApi(
				...
				'/shopify-subscription-version/read'
			);
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\Wallee\Sdk\Model\ShopifySubscriptionVersion', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \Wallee\Sdk\Model\EntityQuery $query The query restricts the Shopify subscription versions which are returned by the search. (required)
	 * @throws \Wallee\Sdk\ApiException
	 * @throws \Wallee\Sdk\VersioningException
	 * @throws \Wallee\Sdk\Http\ConnectionException
	 * @return \Wallee\Sdk\Model\ShopifySubscriptionVersion[]
	...
	public function searchWithHttpInfo($space_id, $query) {
		...
		// path params
		$resourcePath = '/shopify-subscription-version/search';
		// default format to json
		$resourcePath = str_replace('{format}', 'json', $resourcePath);
		...
		try {
			...
			$response = $this->apiClient->callApi(
				...
				$headerParams,
				'\Wallee\Sdk\Model\ShopifySubscriptionVersion[]',
				'/shopify-subscription-version/search'
			);
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\Wallee\Sdk\Model\ShopifySubscriptionVersion[]', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
