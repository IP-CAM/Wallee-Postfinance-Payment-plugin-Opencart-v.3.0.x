...
/**
 * ShopifyRecurringOrderService service
 *
 * @category Class
 * @package  Wallee\Sdk
 * @author   wallee AG
...
class ShopifyRecurringOrderService {
	...
	public function countWithHttpInfo($space_id, $filter = null) {
		...
		// path params
		$resourcePath = '/shopify-recurring-order/count';
	 ...
	 * @throws \Wallee\Sdk\VersioningException
	 * @throws \Wallee\Sdk\Http\ConnectionException
	 * @return \Wallee\Sdk\Model\ShopifyRecurringOrder
	 */
	...
	public function readWithHttpInfo($space_id, $id) {
		...
		// path params
		$resourcePath = '/shopify-recurring-order/read';
		// default format to json
		$resourcePath = str_replace('{format}', 'json', $resourcePath);
		...
		try {
			...
			$response = $this->apiClient->callApi(
				...
				$httpBody,
				$headerParams,
				'\Wallee\Sdk\Model\ShopifyRecurringOrder',
				'/shopify-recurring-order/read'
			);
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\Wallee\Sdk\Model\ShopifyRecurringOrder', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \Wallee\Sdk\Model\EntityQuery $query The query restricts the Shopify recurring orders which are returned by the search. (required)
	 * @throws \Wallee\Sdk\ApiException
	 * @throws \Wallee\Sdk\VersioningException
	 * @throws \Wallee\Sdk\Http\ConnectionException
	 * @return \Wallee\Sdk\Model\ShopifyRecurringOrder[]
	...
	public function searchWithHttpInfo($space_id, $query) {
		...
		// path params
		$resourcePath = '/shopify-recurring-order/search';
		// default format to json
		$resourcePath = str_replace('{format}', 'json', $resourcePath);
		...
		try {
			...
			$response = $this->apiClient->callApi(
				...
				$headerParams,
				'\Wallee\Sdk\Model\ShopifyRecurringOrder[]',
				'/shopify-recurring-order/search'
			);
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\Wallee\Sdk\Model\ShopifyRecurringOrder[]', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \Wallee\Sdk\Model\ShopifyRecurringOrderUpdateRequest $update_request  (required)
	 * @throws \Wallee\Sdk\ApiException
	 * @throws \Wallee\Sdk\VersioningException
	 * @throws \Wallee\Sdk\Http\ConnectionException
	 * @return void
	 */
	public function update($space_id, $update_request) {
		return $this->updateWithHttpInfo($space_id, $update_request)->getData();
	}
	 ...
	 * @return ApiResponse
	 */
	public function updateWithHttpInfo($space_id, $update_request) {
		// verify the required parameter 'space_id' is set
		if (is_null($space_id)) {
		...
		if (is_null($update_request)) {
			throw new \InvalidArgumentException('Missing the required parameter $update_request when calling update');
		}
		// header params
		$headerParams = [];
		$headerAccept = $this->apiClient->selectHeaderAccept(['application/json;charset=utf-8']);
		...
		// path params
		$resourcePath = '/shopify-recurring-order/update';
		// default format to json
		$resourcePath = str_replace('{format}', 'json', $resourcePath);
		...
		try {
			...
			$response = $this->apiClient->callApi(
				...
				$headerParams,
				null,
				'/shopify-recurring-order/update'
			);
