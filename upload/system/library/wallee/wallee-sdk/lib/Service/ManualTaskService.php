...
class ManualTaskService {
	...
	public function countWithHttpInfo($space_id, $filter = null) {
		...
		// path params
		$resourcePath = '/manual-task/count';
	...
	public function readWithHttpInfo($space_id, $id) {
		...
		try {
			...
			$response = $this->apiClient->callApi(
				...
				$httpBody,
				$headerParams,
				'\Wallee\Sdk\Model\ManualTask',
				'/manual-task/read'
			);
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\Wallee\Sdk\Model\ManualTask', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
	...
	public function searchWithHttpInfo($space_id, $query) {
		...
		// path params
		$resourcePath = '/manual-task/search';
		// default format to json
		$resourcePath = str_replace('{format}', 'json', $resourcePath);
