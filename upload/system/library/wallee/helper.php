...
class WalleeHelper {
	...
	public function getCurrentCartId(){
		if (isset($this->registry->get('session')->data['cart_id'])) {
			return $this->registry->get('session')->data['cart_id'];
		}
		
		$table = DB_PREFIX . 'cart';
		$api_id = (isset($this->registry->get('session')->data['api_id']) ? (int) $this->registry->get('session')->data['api_id'] : 0);
		$customer_id = (int) $this->registry->get('customer')->getId();
		$session_id = $this->registry->get('session')->getId();
		$session_id = $this->registry->get('db')->escape($session_id);
		$customer_id = $this->registry->get('db')->escape($customer_id);
		$api_id = $this->registry->get('db')->escape($api_id);
	...
	public function getSuccessUrl(){
		return self::createUrl($this->getCatalogUrl(), 'checkout/success', '', $this->registry->get('config')->get('config_secure'));
	}

	public function getFailedUrl($order_id){
		return str_replace('&amp;', '&',
				self::createUrl($this->getCatalogUrl(), 'checkout/checkout', array(
					'order_id' => $order_id 
				), $this->registry->get('config')->get('config_secure')));
	}

	public function getWebhookUrl(){
		return self::createUrl($this->getCatalogUrl(), 'extension/wallee/webhook', '', $this->registry->get('config')->get('config_secure'));
	}
