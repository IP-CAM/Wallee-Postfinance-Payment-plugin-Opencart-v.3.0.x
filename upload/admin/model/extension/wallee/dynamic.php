...
class ModelExtensionWalleeDynamic extends AbstractModel {
		...
		foreach ($stores as $store) {
			...
			foreach ($configurations as $configuration) {
				...
				foreach ($configuration as $partialKey => $value) {
					$settings['payment_' . $configuration['code'] . '_' . $partialKey] = $value;
				}
				$this->model_setting_setting->editSetting('payment_' . $configuration['code'], $settings, $store['store_id']);
	...
	private function createLanguageFiles(array $configuration, $type){
		...
		foreach ($languages as $code => $language) {
			...
			$target = str_replace('#code#', $language['directory'], $target_base);
