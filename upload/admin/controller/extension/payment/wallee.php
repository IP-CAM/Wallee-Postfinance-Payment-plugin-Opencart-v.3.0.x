	...
	private function processPostData($shops){
		...
		foreach ($shops as $store) {
			...
			if (($this->request->server['REQUEST_METHOD'] == "POST") && ($this->validateStore($store['id']))) {
				...
				$post[\Wallee\Service\ManualTask::CONFIG_KEY] = $this->model_setting_setting->getSetting(\Wallee\Service\ManualTask::CONFIG_KEY,
						$store['id']);
				
				// preserve notification url
				$post['wallee_notification_url'] = $this->model_setting_setting->getSetting('wallee_notification_url', $store['id']);
				...
				$this->model_setting_setting->editSetting('payment_wallee', $status, $store['id']);
				$changed = true;
			}
		}
		if ($changed) {
